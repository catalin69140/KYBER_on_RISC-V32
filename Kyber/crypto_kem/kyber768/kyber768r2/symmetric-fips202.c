...
*
* Arguments:   - shake128ctx *s:               pointer to (uninitialized) output SHAKE128 state
                           ...
                           unsigned char y)
{
  ...
  /* Uses the mupq/common/fips202.h SHAKE128 interface */
...
*              - const unsigned char *key:        pointer to key (length KYBER_SYMBYTES)
...
{
  unsigned char extkey[KYBER_SSBYTES + 1];
  size_t i;
